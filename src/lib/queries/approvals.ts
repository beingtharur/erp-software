...
export async function getPendingApprovals(role: AccessRole) {
  const approvals = await prisma.approvalRequest.findMany({
    where: { status: "PENDING", approverRole: role },
  ...
  const purchaseOrders = poIds.length
    ? await prisma.purchaseOrder.findMany({ where: { id: { in: poIds } }, include: { vendor: true } })
    : [];
